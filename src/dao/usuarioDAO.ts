...
export default class UsuarioDAO {
	public static async createUser(data: Omit<Usuario, 'id_usuario'>): Promise<Result<UsuarioCreationResult>> {
		...
		const existingUser = await pool.oneOrNone(SQL_USUARIO.existUserInStore, [username, id_tienda]);
		...
		try {
			const result: UsuarioCreationResult = await pool.task(async (consulta) => {
				return await consulta.one<UsuarioCreationResult>(SQL_USUARIO.createUser, [username, password, id_tienda, id_rol]);
			});
	...
	public static async fetchUsers(tienda: number): Promise<Result<Usuario[]>> {
		try {
			const result: Usuario[] = await pool.manyOrNone(SQL_USUARIO.getUserList, tienda);
	...
	public static async finAllUsers(): Promise<Result<UsuarioR[]>> {
		try {
			const result: UsuarioR[] = await pool.manyOrNone(SQL_USUARIO.getAllUsersWithStore);
			return Result.success(result);
		} catch (error) {
			return Result.fail(`No se puede obtener los usuarios, ${error}`);
		}
	...
	public static async updateUser(fieldsToUpdate: { [key: string]: any }, idUsuario: number, tienda: number): Promise<Result<void>> {
		...
		const existingUser = await pool.oneOrNone(SQL_USUARIO.existUserInStore, [idUsuario, tienda]);
	...
	public static async deleteUser(tienda_id: string, id: string) {
		const existingUser = await pool.oneOrNone(SQL_USUARIO.findUserByStoreAndId, [id, tienda_id]);
		...
		try {
			await pool.query(SQL_USUARIO.deleteUser, [id, tienda_id]);
			return Result.success();
		} catch (error: any) {
